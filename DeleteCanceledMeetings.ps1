...
## EWS URL
$EWSURL = "https://objplcas1.objectivity.co.uk/ews/exchange.asmx"
...
foreach($Item in $fiItems.Items){ 
    ...
    if ($Item.IsCancelled -eq "True") {
     ...
     #############################
        ...
        "$nowlog INFO: Spotkanie '"+ $Item.Subject + "' zosta³o usuniête z kalendarza $RoomName ("+$Item.Start+" - "+$Item.End+")" >> $reportlog
        } else {
        Write-Host "$nowlog INFO: Spotkanie '" $Item.Subject "' zostanie usuniête z kalendarza" $RoomName "("$Item.Start "-" $Item.End")"
        }
    }
